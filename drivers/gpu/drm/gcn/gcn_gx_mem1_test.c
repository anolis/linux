...
#define TEST_START		0x01300000ULL
#define TEST_SIZE		0x00200000ULL
#define TEST_ALIGNMENT		32ULL
#define TEST_WORKSPACE_SIZE	(768 * 1024ULL)
...
static void gcn_gx_mem1_places_and_reuses_workspaces(struct kunit *test)
{
	struct gcn_gx_mem1_allocator allocator = {};
	...
	KUNIT_ASSERT_EQ(test, 0,
		gcn_gx_mem1_allocator_init(&allocator, TEST_START,
					   TEST_SIZE, TEST_ALIGNMENT));
	KUNIT_ASSERT_EQ(test, 0,
		gcn_gx_mem1_insert(&allocator, &first, TEST_WORKSPACE_SIZE,
				   TEST_ALIGNMENT));
	KUNIT_ASSERT_EQ(test, 0,
		gcn_gx_mem1_insert(&allocator, &second, TEST_WORKSPACE_SIZE,
				   TEST_ALIGNMENT));
	KUNIT_EXPECT_EQ(test, TEST_START, first.start);
	KUNIT_EXPECT_EQ(test, TEST_START + TEST_WORKSPACE_SIZE, second.start);
	...
	KUNIT_ASSERT_EQ(test, 0,
		gcn_gx_mem1_insert(&allocator, &replacement,
				   TEST_WORKSPACE_SIZE, TEST_ALIGNMENT));
...
static void gcn_gx_mem1_enforces_capacity(struct kunit *test)
{
	...
	struct drm_mm_node first = {};
	struct drm_mm_node second = {};
	struct drm_mm_node spare = {};
	struct drm_mm_node overflow = {};
	u64 spare_size = TEST_SIZE - 2 * TEST_WORKSPACE_SIZE;
	...
	KUNIT_ASSERT_EQ(test, 0,
		gcn_gx_mem1_insert(&allocator, &second, TEST_WORKSPACE_SIZE,
				   TEST_ALIGNMENT));
	KUNIT_ASSERT_EQ(test, 0,
		gcn_gx_mem1_insert(&allocator, &spare, spare_size,
				   TEST_ALIGNMENT));
	KUNIT_EXPECT_EQ(test, TEST_START + 2 * TEST_WORKSPACE_SIZE,
			spare.start);
	KUNIT_EXPECT_EQ(test, -ENOSPC,
		gcn_gx_mem1_insert(&allocator, &overflow, TEST_ALIGNMENT,
				   TEST_ALIGNMENT));

	gcn_gx_mem1_remove(&spare);
	gcn_gx_mem1_remove(&second);
	gcn_gx_mem1_remove(&first);
	gcn_gx_mem1_allocator_fini(&allocator);
}
...
static struct kunit_case gcn_gx_mem1_test_cases[] = {
	...
	KUNIT_CASE(gcn_gx_mem1_places_and_reuses_workspaces),
