				 ...
				 u16 rect_width, u16 rect_height, u16 color)
{
	...
	mutex_lock(&gcn_drm_accel_lock);
	if (gcn_drm_accel && gcn_drm_accel->fill_system_rgb565)
		ret = gcn_drm_accel->fill_system_rgb565(dst, width, height,
				dst_layout, x, y, rect_width, rect_height, color);
	mutex_unlock(&gcn_drm_accel_lock);
				...
				const struct gcn_drm_depth_state *depth)
{
	...
	mutex_lock(&gcn_drm_accel_lock);
	if (gcn_drm_accel == provider && provider->draw_triangles_depth_rgb565)
		ret = provider->draw_triangles_depth_rgb565(dst_allocation, width,
				height, vertices, triangle_count, state, depth);
	mutex_unlock(&gcn_drm_accel_lock);
				   ...
				   const struct gcn_drm_draw_state *state)
{
	...
	if (gcn_drm_accel == provider &&
	    provider->draw_textured_triangles_rgb565)
		ret = provider->draw_textured_triangles_rgb565(src_allocation,
				  ...
				  const struct gcn_drm_draw_state *state)
{
	...
	mutex_lock(&gcn_drm_accel_lock);
	if (gcn_drm_accel == provider &&
	    provider->draw_indexed_triangles_rgb565)
				 ...
				 const struct gcn_drm_depth_state *depth)
{
	...
	mutex_lock(&gcn_drm_accel_lock);
	if (gcn_drm_accel == provider &&
	    provider->draw_itex_depth_rgb565)
		ret = provider->draw_itex_depth_rgb565(src_allocation,
				dst_allocation,
				src_width, src_height, dst_width, dst_height,
				vertices, vertex_count, indices, triangle_count,
				state, depth);
