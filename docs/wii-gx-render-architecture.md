...
This document defines the next development stages after commit `e5530616a`.
It is a design record, not a frozen userspace ABI.
...
That last result proves the minimum hardware loop required for reusable render
targets. It does not by itself provide safe buffer allocation, multi-client
isolation, a render node, or an OpenGL driver.
...
The texture window holds only two 640x576 RGB565 surfaces. It cannot be
treated as an unlimited heap. The current alternating scanout uploads consume
both slots even though the synchronous DRM callback generally needs only one.
Changing that partition requires a measured scanout regression and must not
silently consume the OHCI, FIFO, XFB, or wrapper safety gaps.
...
The next code branch should implement only Stage 1 resource description and
validation. Its acceptance gates are:

- device-tree schema and build checks pass;
- no hard-coded texture/FIFO address remains in operational code;
- the allocator cannot overlap FIFO, XFB, OHCI, kernel, or wrapper memory;
- module load/unload and absent-provider CPU fallback remain unchanged; and
- all previously accepted Wii hardware tests pass with checksum-pinned
  artifacts.

Only after those gates pass should the project draft `include/uapi/drm/gcn_drm.h`.
