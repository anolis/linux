...
static void restore_base_frame(uint8_t *frame, const uint8_t *templates,
			       unsigned int stride, unsigned int height)
{
	unsigned int y;

	for (y = 0; y < height; y++) {
		unsigned int template = y % 60 < 2 ? 2 : ((y / 16) & 1);

		memcpy(frame + (size_t)y * stride,
		       templates + (size_t)template * stride, stride);
	}
}
		       ...
		       unsigned int height, uint64_t frame_number)
{
	...
	unsigned int lower_top = height / 2 + 8;
	unsigned int lower_range = height - lower_top - 20;

	restore_base_frame(frame, templates, stride, height);
